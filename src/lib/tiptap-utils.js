...
export const handleImageUpload = async (file, onProgress, abortSignal) => {
  ...
  // If Cloudinary env vars are present, use them. Otherwise fall back to simulated upload.
  const cloudName = process?.env?.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME
  const uploadPreset = process?.env?.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET

  if (!cloudName || !uploadPreset) {
    // Simulate upload (keeps previous behavior so dev without Cloudinary still works)
  ...
  // Use XMLHttpRequest to enable upload progress events and abort support
    ...
    xhr.onreadystatechange = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        try {
          ...
          if (resp?.secure_url) return resolve(resp.secure_url)
          // Cloudinary sometimes returns different shapes; resolve with full response if URL missing
          return resolve(resp?.url || resp?.secure_url || JSON.parse(xhr.responseText))
        } catch (e) {
          return reject(new Error('Invalid response from upload service'))
        }
      }
